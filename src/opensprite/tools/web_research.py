...
class WebResearchTool(Tool):
    ...
    async def _execute(
        ...
        **kwargs: Any,
        ...
        search_result = await self.search_tool._execute(query=query, count=search_count, freshness=effective_freshness)
        search_payload = _parse_json_object(search_result)
        if search_payload is None:
            return _research_payload(
                query=query,
                freshness=effective_freshness,
                search_provider=getattr(self.search_tool, "provider", ""),
                search_items=[],
                fetched_sources=[],
                failed_sources=[{"reason": str(search_result or "web_search returned no structured result")[:500]}],
            )

        search_items = _dedupe_search_items(_coerce_search_items(search_payload), limit=search_count)
        fetched_sources: list[dict[str, Any]] = []
        failed_sources: list[dict[str, Any]] = []
        source_records: list[dict[str, Any]] = []
        search_provider = str(search_payload.get("provider") or getattr(self.search_tool, "provider", "") or "")

        for item in search_items:
            source_records.append({**item, "tool_name": "web_search", "fetched": False})
...
def _merge_fetch_source(
    ...
    *,
    ...
    return {
        ...
        "has_title": bool(_clean_text(fetch_payload.get("title") or item.get("title"))),
