"""Structured task progress planning and state updates."""
        ...
        *,
        ...
        return WorkProgressUpdate(
            ...
            next_action=self._next_action(
                completion_result,
                has_progress=bool(signals),
                attempts=auto_continue_attempts,
    ...
    *,
    ...
    for update in normalized_updates:
        if not update.task_id:
            continue
        previous = by_id.get(update.task_id)
...
def _map_state_status(completion_result: CompletionGateResult, _progress: WorkProgressUpdate) -> str:
    if is_complete_completion_status(completion_result.status):
