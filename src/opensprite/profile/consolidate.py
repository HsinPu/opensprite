"""Global USER.md profile consolidation."""
...
def _format_messages(messages: list[dict[str, Any]]) -> str:
    """Render stored messages into a compact transcript for profiling."""
    lines: list[str] = []
...
class UserProfileConsolidator:
    ...
    def _to_message_dict(message: StoredMessage) -> dict[str, Any]:
        """Convert stored messages into plain dicts for consolidation."""
        return {
            "role": message.role,
    ...
    async def maybe_update(self, chat_id: str) -> None:
        """Update the global USER.md profile when enough new messages exist."""
        if not self.enabled:
            return
