...
def test_core_coding_subagents_are_executable_workflows(tmp_path: Path) -> None:
    ...
    expectations = {
        "implementer": ("implementation", ["apply_patch", "Verification", "不要只輸出建議方案"]),
        "debugger": ("implementation", ["重現", "Root Cause", "直接修正"]),
        "code-reviewer": ("read-only", ["Review Findings", "實際變更", "一般 review 預設只回報 findings"]),
        "test-writer": ("testing", ["直接新增或修改測試", "Verification", "regression test"]),
    }

    for prompt_type, (expected_profile, required_snippets) in expectations.items():
